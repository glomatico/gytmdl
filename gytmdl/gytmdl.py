...
class Gytmdl:
    ...
    def get_tags(self, ytmusic_watch_playlist):
        ...
        tags = {
            ...
            "\xa9ART": [
                self.get_artist(ytmusic_watch_playlist["tracks"][0]["artists"])
            ],
            "\xa9cmt": [f"https://music.youtube.com/watch?v={video_id}"],
            "covr": [
                MP4Cover(
                    self.get_cover(
                        f'{ytmusic_watch_playlist["tracks"][0]["thumbnail"][0]["url"].split("=")[0]}=w1200-l94'
                    )
                )
            ],
        ...
        track_number = 1
        for video in self.get_ydl_extract_info(
    ...
    def get_final_location(self, tags):
        return (
            ...
            / self.get_sanizated_string(tags["©alb"][0], True)
            / (
                self.get_sanizated_string(
